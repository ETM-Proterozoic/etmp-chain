...
func (h *Header) unmarshalRLPFrom(_ *fastrlp.Parser, v *fastrlp.Value) error {
	...
	// miner
	// if h.Miner, err = elems[2].GetBytes(h.Miner.Bytes()); err != nil { // Todo: !!! Sure
	// 	return err
	// }
	// stateroot
...
func (t *Transaction) UnmarshalRLP(input []byte) error {
	...
	if len(input) > 0 && input[0] <= RLPSingleByteUpperLimit {
		var err error
		if t.Type, err = txTypeFromByte(input[0]); err != nil { //Todo: Important
			return err
		}
...
func (t *Transaction) unmarshalRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
	...
	// S
	t.S = new(big.Int)
	if err = elems[8].GetBigInt(t.S); err != nil { //Todo: Important error is here
		return err
	}
