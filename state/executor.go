...
func (t *Transition) apply(msg *types.Transaction) (*runtime.ExecutionResult, error) {
	...
	// // 2. caller has enough balance to cover transaction fee(gaslimit * gasprice)  //Todo: temp remove
	...
	// refund the sender
	remaining := new(big.Int).Mul(new(big.Int).SetUint64(result.GasLeft), gasPrice) //Todo: big doubt why not compare gasFeeCap and gasTipCap
