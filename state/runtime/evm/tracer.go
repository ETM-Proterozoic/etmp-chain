...
import (
	...
	"errors"
	"fmt"
	"github.com/0xPolygon/polygon-edge/common"
	"github.com/0xPolygon/polygon-edge/common/hexutil"
	"github.com/0xPolygon/polygon-edge/crypto"
	"github.com/0xPolygon/polygon-edge/state/runtime"
	"github.com/0xPolygon/polygon-edge/state/runtime/evm/internal/tracers"
...
type JSTracer struct {
	...
	//env *runtime.Runtime // EVM instance executing the code being traced
...
func NewJsTracer(code string, ctx Transition) (*JSTracer, error) {
	...
	if ctx.Block().Hash() != (types.Hash{}) {
		tracer.ctx["blockHash"] = ctx.Block().Hash()

		if ctx.GetTxnHash() != (types.Hash{}) {
			txIndex := 0
			block := ctx.Block()
			for i := 0; i < len(block.Transactions); i++ {
				txIndex = i
				if block.Transactions[txIndex].Hash == ctx.GetTxnHash() {
					break
				}
			}
			tracer.ctx["txIndex"] = txIndex
			tracer.ctx["txHash"] = ctx.GetTxnHash()
		}
	}
	...
	tracer.vm.PushGlobalGoFunction("toContract", func(ctx *duktape.Context) int {
		...
		if ptr, size := ctx.GetBuffer(-2); ptr != nil {
			...
			from = types.StringToAddress(ctx.GetString(-2))
		}
	...
	tracer.vm.PushGlobalGoFunction("toContract2", func(ctx *duktape.Context) int {
		...
		if ptr, size := ctx.GetBuffer(-3); ptr != nil {
			...
			from = types.StringToAddress(ctx.GetString(-3))
		}
...
func (jst *JSTracer) CaptureStart(from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	//jst.env = env
	jst.ctx["type"] = "CALL"
	if create {
		jst.ctx["type"] = "CREATE"
...
func (jst *JSTracer) CaptureState(pc uint64, op int, gas, cost uint64, scope runtime.ScopeContext, rData []byte, depth int, err error) {
	...
	jst.opWrapper.op = OpCode(op)
	data := make([]big.Int, len(scope.Stack))
	for i, v := range scope.Stack {
		data[i] = *v
	}
	jst.stackWrapper.stack = &Stack{data: data}

	store := scope.Memory
	jst.memoryWrapper.memory = Memory{store: store, lastGasCost: cost}
...
func (jst *JSTracer) CaptureEnter(typ int, from common.Address, to common.Address, input []byte, gas uint64, value *big.Int) {
