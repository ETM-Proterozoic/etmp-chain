...
import (
	...
	//   "github.com/0xPolygon/polygon-edge/eth/tracers"
...
func (t *jsTracer) CaptureStart(txr interface{}, from types.Address, to types.Address, create bool, input []byte, gas uint64, value *big.Int) {
	// t.env = env
	tn, ok := txr.(*state.Transition)
	if !ok {
		return
	}
	t.transition = tn

	// db := &dbObj{db: txn.State, vm: t.vm, toBig: t.toBig, toBuf: t.toBuf, fromBuf: t.fromBuf}
	db := &dbObj{db: t.transition.GetState(), vm: t.vm, toBig: t.toBig, toBuf: t.toBuf, fromBuf: t.fromBuf}
	...
	t.ctx["block"] = t.vm.ToValue(t.transition.ContextPtr().Number)
	// Update list of precompiles based on current block
	//Todo: del，seems it is only effect the geth not for the polygon
...
// func (t *jsTracer) CaptureState(pc uint64, op evm.OpCode, gas, cost uint64, scope *runtime.ScopeContext, rData []byte, depth int, err error) {
func (t *jsTracer) CaptureState(pc uint64, op int, gas, cost uint64, scope *runtime.ScopeContext, rData []byte, depth int, err error) {
	if !t.traceStep {
		return
	}
	if t.err != nil {
		fmt.Println("CaptureState ------------------- err ", t.err)
		return
	}
	...
	if log.op.op == evm.CREATE2 {
		fmt.Printf(" CaptureState #####  %+v     %s    %s \n", *(t.obj), t.logValue.String(), t.dbValue.String())
...
// func (t *jsTracer) CaptureFault(pc uint64, op evm.OpCode, gas, cost uint64, scope *runtime.ScopeContext, depth int, err error) {
...
// func (t *jsTracer) CaptureEnter(typ evm.OpCode, from types.Address, to types.Address, input []byte, gas uint64, value *big.Int) {
...
func (co *contractObj) GetInput() goja.Value {
	input := types.CopyBytes(co.contract.Input)
